...
| **gsd-planner.md** | 项目规划、任务拆分、进度跟踪、风险管理、质量把关 | 输出: `PLAN.md`, 进度报告、风险评估 |
...
```
业务分析师  →  项目经理  →  架构师  →  (开发并行)  →  测试工程师  →  E2E测试  →  部署工程师
  (需求)      (计划跟踪)   (架构)     (编码实现)    (单元/集成)     (端到端)   (CI/CD)
                 ↓           ↓           ↓             ↓            ↓          ↓
                 └─────────────────────────────────────────────────────────────┘
                              (全程监督、风险管理、进度把控)
```
...
claude agent gsd-planner           # 项目规划与进度管理（项目经理）
